...
```
Kiro reads its instructions from the file. Can read files and execute shell commands autonomously. The parent Claude Code session's auto-mode classifier does NOT intercept commands Kiro runs internally — diff-borne prompt injection can still execute here.
