...
- **Graceful no-op.** With `headroom` off PATH OR `COMMITTEE_HEADROOM=off`, behavior is byte-identical to today.
- **`headroom` is optional** — do NOT add it to `spawn.sh`'s required-tool preflight loop (line ~151).
...
- **Opt-out var:** `COMMITTEE_HEADROOM` — any value other than `off` (incl. unset) means "auto" (wrap when installed).

---

### Task 1: `build_inner_launch` helper + self-test hook + wiring in `spawn.sh` (with smoke test)

**Files:**
- Create: `scripts/headroom-launch-smoke.sh`
- Modify: `.claude/skills/committee-loop/spawn.sh` (insert helper+hook after line 27; replace launch at lines 373–374)

**Interfaces:**
- Produces: `build_inner_launch "<claude-args-string>"` → prints `headroom wrap claude --no-serena -- <claude-args-string>` when `headroom` is on PATH and `COMMITTEE_HEADROOM != off`, else `claude <claude-args-string>`. Pure (depends only on `$1` + `PATH` + `COMMITTEE_HEADROOM`).
...
# Case 2: opt-out via COMMITTEE_HEADROOM=off -> bare claude (even with headroom present)
got=$(PATH="$BASEPATH" COMMITTEE_HEADROOM=off bash "$SPAWN" --print-inner-launch "$ARGS")
check "$got" "claude $ARGS" "COMMITTEE_HEADROOM=off -> bare claude"
...
# `headroom` CLI is installed (and not opted out via COMMITTEE_HEADROOM=off), wrap
# the launch with `headroom wrap claude` so the inner coordinator AND every
# in-harness Claude agent it spawns (the /committee Claude reviewer + all
...
- [ ] **Step 4: Wire the tmux launch to use the helper**
...
Run: `bash scripts/headroom-launch-smoke.sh`
Expected: PASS — three `PASS:` lines + `ALL SMOKE CHECKS PASSED`, exit 0.

- [ ] **Step 6: Verify the bash still parses cleanly**
...
- Modify: `prompts/reviewers/claude.md` (add a paragraph after line 5)
- Modify: `.claude/skills/committee-loop/inner-agent.md` (add a bullet to step 3's "Each verifier:" list, after the "Returns a decision proposal" bullet ~line 127)
...
In "## Your Task" → step 3 list, add this as a new `-` item immediately after the existing "Third-party SDK/API-surface claims" bullet (the one ending "…do not let it stand as Critical."):
...
Immediately after line 5 (the "**You are a REVIEWER, not an implementer.**" paragraph) and before "## Review Lens", insert:
...
In "### 3. Classify with streaming verifier dispatch" → the "Each verifier:" list, add a new `-` bullet immediately after the "Returns a decision proposal per finding with its verification evidence" bullet:
...
**Files:**
- Modify: `.claude/skills/committee/SKILL.md` (add a routing-check note in "## Progress notification", ~line 113–119)
- Modify: `.claude/skills/committee-loop/SKILL.md` (add a bullet in "## Notes", ~line 212)
- Modify: `CLAUDE.md` (add a "## Headroom integration" section after "## Architectural Notes")
...
At the END of the "## Progress notification" section (after the "A single commit is ~5–8 min…" line), append:
...
```bash
{ [ -n "${ANTHROPIC_BASE_URL:-}" ] && command -v headroom >/dev/null 2>&1 && echo routed; } || true
```

- If it prints `routed`: add a line to the user — *"Claude reviewer + verifiers are routing through Headroom ✓ (your session is wrapped)."*
- Otherwise: add — *"(tip: launch your session via `headroom wrap claude` to route committee's Claude-side reviewers through Headroom — Codex/Kiro/Gemini stay native.)"*
...
- **committee-loop:** `spawn.sh`'s `build_inner_launch` launches the detached inner coordinator via `headroom wrap claude --no-serena -- <claude args>` (reuses a running proxy, or starts one). Because in-harness subagents inherit the session's `ANTHROPIC_BASE_URL`, this routes the coordinator **and** the `/committee` Claude reviewer **and** all five per-reviewer verifiers automatically. Set `COMMITTEE_HEADROOM=off` to force bare `claude`. `--no-serena` skips Serena registration (unused, and sidesteps an error on hosts without Serena); the Headroom MCP stays registered so `headroom_retrieve` is available; `--tool-search` keeps its default `true` so Claude Code's deferred tool-loading is preserved (a custom `ANTHROPIC_BASE_URL` otherwise makes Claude Code eagerly load every tool schema — Headroom issue #746).
- **one-shot `/committee`:** the coordinator is your live session, which committee can't re-wrap — launch it yourself via `headroom wrap claude` to route the Claude reviewer + verifiers. The skill detects (`ANTHROPIC_BASE_URL` set + `headroom` on PATH) and reports routing status; behavior is identical either way.
...
### Task 4: Live verification (wrap probe, readiness window, end-to-end)

**Files:**
- Possibly modify: `.claude/skills/committee-loop/spawn.sh:378` (`for _ in $(seq 1 30)` → widen ONLY if the live test shows the wrapped TUI doesn't render in 30s)

**Interfaces:**
- Consumes: `build_inner_launch` (Task 1).
- Produces: confirmation that the real path works; an optional readiness-window bump.

- [ ] **Step 1: Confirm the live wrap path works**

Run (proves `headroom wrap claude --no-serena` answers through the proxy and reuses the running proxy):
```bash
headroom doctor --json | grep -o '"savings"[^}]*' | head -1   # note "before"
headroom wrap claude --no-serena -- -p "reply with the single word OK"
headroom doctor --json | grep -o '"savings"[^}]*' | head -1   # "after" should show fresh traffic
```
Expected: claude prints `OK`; the savings/last-request stat advances. If it errors on `--no-serena`, STOP and report (a Headroom version without that flag — re-evaluate keeping it).

- [ ] **Step 2: End-to-end committee-loop smoke on a trivial target**

Create a throwaway target, run a short loop, and confirm the inner session comes up *wrapped*:
```bash
printf '# scratch\necho hi\n' > /tmp/headroom-e2e-target.sh
cp /tmp/headroom-e2e-target.sh ./headroom-e2e-target.sh
git add headroom-e2e-target.sh && git commit -q -m "test: throwaway headroom e2e target"
bash .claude/skills/committee-loop/spawn.sh headroom-e2e-target.sh
# Capture SESSION from the manifest, then verify the pane shows a wrapped launch:
SESSION=$(grep '^SESSION=' ./*-committee-loop-*/.committee-loop-manifest.txt 2>/dev/null | head -1 | cut -d= -f2- | tr -d "'")
tmux -L committee-loop list-panes -t "$SESSION" -F '#{pane_start_command}' 2>/dev/null
tmux -L committee-loop capture-pane -t "$SESSION" -p | tail -20
```
Expected: `pane_start_command` contains `headroom wrap claude --no-serena`; the pane shows Claude's TUI (trust/bypass footer or input box) — i.e. the readiness poll caught it. Tear down: `tmux -L committee-loop kill-session -t "$SESSION"; git worktree remove --force ./*-committee-loop-* ; git branch -D "$(git branch --list 'committee-loop/*')"` then `git rm -f headroom-e2e-target.sh && git commit -q -m "test: remove throwaway target"`.

- [ ] **Step 3: Widen the readiness window ONLY if needed**

If Step 2 showed the inner TUI did NOT render within 30s under wrap (spawn.sh aborted with "input box did not render within 30s"), edit `.claude/skills/committee-loop/spawn.sh:378`:
...
→
```bash
for _ in $(seq 1 45); do   # wider: `headroom wrap` adds proxy-handshake latency before claude's TUI
```

Re-run Step 2 to confirm. If Step 2 already passed within 30s, SKIP this step (no change).

- [ ] **Step 4: Final smoke re-run + commit any readiness change**

Run: `bash scripts/headroom-launch-smoke.sh && bash -n .claude/skills/committee-loop/spawn.sh && echo OK`
Expected: `ALL SMOKE CHECKS PASSED` + `OK`.

If Step 3 changed spawn.sh:
```bash
git add .claude/skills/committee-loop/spawn.sh
git commit -m "fix(committee-loop): widen inner-session readiness window for headroom wrap latency"
```
...
- Goal 4 (graceful no-op) → Task 1 smoke Cases 2 & 3 + `bash -n`. ✓
- Non-goals (no Codex/Kiro/Gemini routing, no `init`, no proxy ownership) → enforced by Global Constraints + documented in Task 3 Step 3. ✓
- Component A (`spawn.sh`) → Task 1. Component B (one-shot) → Task 3. Component C (prompts) → Task 2. Component D (docs) → Task 3. ✓
- Verification criteria 1–4 → Task 1 (smoke), Task 4 Step 1 (wrap probe), Task 4 Step 2 (e2e), Task 1 smoke Cases 2–3 (graceful absence). ✓

**Placeholder scan:** No TBD/TODO; all code blocks are complete; the only `<…>` are literal argument placeholders inside shown commands. ✓

**Type/name consistency:** `build_inner_launch`, `--print-inner-launch`, `COMMITTEE_HEADROOM`, `INNER_LAUNCH`, and the exact wrap string `headroom wrap claude --no-serena -- …` are used identically across Tasks 1, 3, 4 and the smoke test. ✓
