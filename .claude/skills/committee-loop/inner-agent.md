...
- **A 429 / rate-limit error surfaced by one of YOUR tools** — a `general-purpose` subagent, the `/committee` workflow, a verifier, or the Codex/Kiro/agy CLIs failing with a rate-limit / quota / "resets in N hours" message — is **transient**, not a finding and not a BLOCKED. Retry it with **exponential backoff** (~15s, ~30s, ~60s, ~120s …; space the waits with a background shell, not a foreground `sleep`) until it succeeds. Do NOT write `.committee-loop-BLOCKED.txt`, do NOT converge, do NOT route it to DEFERRED, and do NOT emit `<promise>REVIEW CLEAN</promise>` on account of it.
...
b. `timeout 1800 kiro-cli chat --no-interactive --trust-tools=fs_read` with the same prompt listing every target path. Write output to `.committee-loop-iter1-kiro.txt`. On timeout, `rm -f .committee-loop-iter1-kiro.txt` before synthesis so a partial report isn't parsed; 2-of-3 quorum from Claude+Codex still holds.

c. `timeout 1800 codex exec --skip-git-repo-check --sandbox read-only -c model_reasoning_effort=high -o .committee-loop-iter1-codex.txt "Review <TARGET...> ..."` (same prompt, explicit `high` reasoning — user's global config may default to `xhigh` which takes ~2× as long with negligible findings-quality gain for this review scale). The 30m wall-clock cap prevents a hung Codex from blocking iter-1 indefinitely; on timeout, `rm -f .committee-loop-iter1-codex.txt` before synthesis to avoid parsing a truncated report — 2-of-3 quorum from Claude+Kiro still holds.
